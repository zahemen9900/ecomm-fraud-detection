...
def plot_categorical_distribution(df: pd.DataFrame, 
    ...
    if any(isinstance(x, str) for x in df[feature]):
        ...
        df[feature] = df[feature].str.replace("\\&", "\&", regex=False) # correct escaped ampersands
        df[feature] = df[feature].str.replace("\\%", "\%", regex=False)  # correct escaped percent signs
        df[feature] = df[feature].str.replace("\\$", "\$", regex=False)  # correct escaped dollar signs       
