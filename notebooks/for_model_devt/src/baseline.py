...
def plot_feature_importance(feature_importance: pd.DataFrame,
    ...
    # Escape special characters in feature names for LaTeX compatibility
    if 'Feature' in feature_importance.columns:
        # Replace special characters with their escaped versions
        feature_importance['Feature'] = feature_importance['Feature'].astype(str).map(
            lambda x: x.replace('&', 'and').replace('$', '\$').replace('%', '\%')
        )
